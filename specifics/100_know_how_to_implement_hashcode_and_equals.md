...
This is clearly far less effort than hand crafting a complete `Animial` class, but there are some downsides.
...
Here Google Auto introduces some *friction* as the code shown above will not compile until the generator has run to produce the `AutoValue_Animal` class.

There is also some *surprise*.

Animal is conceptually a final class, but we have been forced to make it abstract.
